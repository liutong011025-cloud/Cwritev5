...
export async function POST(request: NextRequest) {
  try {
    ...
    // 构建邮件内容
    const emailSubject = `Letter from Cwrite: To ${recipient}`
    const emailHtml = `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    const emailText = `Letter from Cwrite
...
Created with Cwrite
    ...
    try {
      const resendResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Cwrite <noreply@museaiwrite.com>', // 需要配置发送域名
          to: [to],
